...
interface LeadFormArboProps {
    imovel: ImovelArbo;
}

export const LeadFormArbo = ({ imovel }: LeadFormArboProps) => {
    const tipoInteresse = imovel.transaction_type === 'For Sale' ? 'Venda' : 'Locação';

    const [formData, setFormData] = useState({
        nome: '',
        telefone: '',
        email: '',
        tipo_interesse: tipoInteresse as 'Venda' | 'Locação',
        termos: false,
    });
    const [loading, setLoading] = useState(false);
    const [viewCount] = useState(Math.floor(Math.random() * 50) + 20);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Criar lead - usa observacoes para registrar info do imóvel Arbo
            // Após aplicar migration, pode-se usar imovel_arbo_id diretamente
            const leadPayload: any = {
                ...
                observacoes: `[Vitrine Pública] Imóvel: ${imovel.listing_id} - ${imovel.title || imovel.property_type} | ${imovel.neighborhood}, ${imovel.city}`,
            };
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    title: `Imóvel ${imovel.listing_id}`,
                    url: url,
                });
            } catch (err) {
            ...
            <CardContent className="p-6 space-y-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <Button
                        ...
                        className="w-full text-base py-6 font-semibold"
                        onClick={() => {
                            const whatsappMessage = `Olá, gostaria de agendar uma visita ao imóvel ${imovel.listing_id}`;
